...
APP_ENTITLEMENTS="Source/MIDIBlinker.entitlements"
APP=./MIDIBlinker.xcarchive/Products/Applications/MIDIBlinker.app
...
if [ "${ACTION}" == "package" ]; then
    ...
    codesign -s "${DEV_ID_APP}" -f --timestamp -o runtime \
        --entitlements "${APP_ENTITLEMENTS}" tmp_app/MIDIBlinker.app

    # Build component package
    pkgbuild --root tmp_app --identifier com.larrymtaylor.MIDIBlinker \
    ...
    productbuild --sign "${DEV_ID_INST}" --distribution dist.plist \
        --product requirements.plist MIDIBlinker.pkg
    ...
    xcrun notarytool submit MIDIBlinker.pkg --keychain-profile \
        "LT_PASSWORD" --wait

    # Staple notary ticket to product
    xcrun stapler staple MIDIBlinker.pkg
